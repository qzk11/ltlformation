...
class ltl_planner(object):
    ...
    def find_next_move(self):
        ...
        if self.segment == 'line' and self.index < len(self.run.pre_plan)-2:
            ...
            self.index += 1
            self.next_move = self.run.pre_plan[self.index]
            self.guard = self.run.pre_guard[self.index - 1]
            ...
            self.segment = 'loop'
            self.next_move = self.run.suf_plan[self.index]
            self.guard = self.run.suf_guard[self.index - 1]
    ...
    def updatePlanner(self, current):
        MotionFts = self.product.graph['ts'].graph['region']
        MotionFts.set_initial(current)
        MotionFts.add_full_edges()
        self.product.graph['ts'].graph['initial'].clear()
        self.product.graph['ts'].remove_edges_from([edge for edge in self.product.graph['ts'].edges])
        self.product.graph['ts'].remove_nodes_from([node for node in self.product.graph['ts'].nodes])
        self.product.graph['initial'].clear()
        self.product.remove_edges_from([edge for edge in self.product.edges])
        self.product.remove_nodes_from([node for node in self.product.nodes])
        self.optimal()
