            ...
            for f_buchi_node in self.graph['buchi'].nodes():
                ...
                for t_ts_node in self.graph['ts'].successors(f_ts_node):
                    for t_buchi_node in self.graph['buchi'].successors(f_buchi_node):
                        ...
                        if truth:
                            guard = self.graph['buchi'].edges[f_buchi_node, t_buchi_node]['guard']
                            self.add_edge(f_prod_node, t_prod_node, weight=total_weight, guard=guard)
    ...
    def fly_successors(self, f_prod_node):
        ...
        if ((self.nodes[f_prod_node]['marker'] == 'visited') and
            (self.graph['ts'].graph['region'].node[
                self.graph['ts'].node[self.nodes[f_prod_node]['ts']]['region']]['status'] == 'confirmed')):
...
class ProdAut_Run(object):
    ...
    def plan_output(self, product):
        ...
        self.pre_guard = []
        for i in xrange(0, self.line.__len__() - 1):
            self.pre_guard.append(product.edges[self.prefix[i], self.prefix[i + 1]]['guard'])
        self.suf_guard = []
        for i in xrange(0, self.loop.__len__() - 1):
            self.suf_guard.append(product.edges[self.suffix[i], self.suffix[i + 1]]['guard'])
        if len(self.line) == 2:
        ...
        print self.pre_plan
        print self.suf_plan
        print self.pre_guard
        print self.suf_guard
